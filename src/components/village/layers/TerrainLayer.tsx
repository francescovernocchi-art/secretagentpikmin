...
interface Props { seed: string; biome: BiomeKey; showGrid?: boolean; }

/** PRNG semplice per jitter cromatico deterministico. */
...
/**
 * Terreno organico: niente bordi tile, niente griglia tecnica. Ogni cella ha
 * una sfumatura leggera deterministica + un overlay di decorazioni naturali
 * (ciuffi d'erba, fiori, sassolini) sparsi sul mondo.
 */
export function TerrainLayer({ seed, biome, showGrid = false }: Props) {
  const tiles = useMemo(() => generateTiles(seed, biome), [seed, biome]);
  const rnd = useMemo(() => jitter(`${seed}|deco|${biome}`), [seed, biome]);

  // Decorazioni naturali sparse (deterministiche)
  const decor = useMemo(() => {
  ...
  return (
    ...
    >
      <defs>
        <radialGradient id="village-vignette" cx="50%" cy="55%" r="65%">
          <stop offset="60%" stopColor="rgba(0,0,0,0)" />
          <stop offset="100%" stopColor="rgba(0,0,0,0.35)" />
        </radialGradient>
      </defs>

      {/* base color uniforme per bioma */}
      <rect x={0} y={0} width={WORLD_W} height={WORLD_H}
        fill={TILE_PALETTE[tiles[0]?.kind ?? "grass"].fill} />

      {/* macchie di colore organiche (accent/rare) — tile senza bordi, senza griglia visibile */}
      {tiles.map((t, i) => {
        const p = TILE_PALETTE[t.kind];
        // Solo i tile diversi dalla base creano "macchie" organiche con leggero overlap
      ...
      {/* griglia opzionale (modalità costruzione) */}
      {showGrid && tiles.map((t, i) => (
        <rect key={`g${i}`} x={t.col * TILE} y={t.row * TILE} width={TILE} height={TILE}
          fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
      ))}

      {/* decorazioni naturali — ciuffi, fiori, sassolini */}
      {decor.map((d, i) => {
        if (d.kind === "grass") {
          return (
            <g key={i} opacity={0.7}>
      ...
      {/* vignetta morbida per dare profondità */}
      <rect x={0} y={0} width={WORLD_W} height={WORLD_H} fill="url(#village-vignette)" />
    </svg>
  );
}
